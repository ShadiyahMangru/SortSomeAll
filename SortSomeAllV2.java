	...
	public void selectPlayerMenu(){
		...
		if(choice <= playersArray.length && choice > 0){
			...
			if(sel.getPosition().equals("Goalie")){
				Goalie g = (Goalie)sel;
				for(int i = 0; i< StatsTracking.goalieStats.length; i++){
					if(i<4){	
						System.out.println("\t\t" + StatsTracking.goalieStats[i] + ": " + (Integer)g.getStatsList().get(i+1));
					}
					else{
						System.out.println("\t\t" + StatsTracking.goalieStats[i] + ": " + (Double)g.getStatsList().get(i+1));
					}
				}
			}
			else{
				Skater s = (Skater)sel;
				for(int i = 0; i< StatsTracking.skaterStats.length; i++){
					if(i<5){	
						System.out.println("\t\t" + StatsTracking.skaterStats[i] + ": " + (Integer)s.getStatsList().get(i+1));
					}
					else{
						System.out.println("\t\t" + StatsTracking.skaterStats[i] + ": " + (Double)s.getStatsList().get(i+1));
					}
				}
			}
			System.out.println("\n\t*** WSH Career Stats ***");
	...
	public void goaliesStatsMenu(){
		...
		if(choice <= 4){
			for(Goalie g : goalieStat)
				System.out.println(g + "'s " + StatsTracking.goalieStats[choice-1] + " : " + (Integer)g.getStatsList().get(choice)); 
		}
		else if(choice > 4 & choice < 7){
			for(Goalie g : goalieStat)
				System.out.println(g + "'s " + StatsTracking.goalieStats[choice-1] + " : " + (Double)g.getStatsList().get(choice)); 
		} 
		switch(choice){
