...
interface StatsTracking{
	...
	public static final int numGIntStats = 4;
	public static final int numSIntStats = 5;
...
class Goalie extends HockeyPlayer implements Comparable<Goalie>{
	...
	private ArrayList<Object> statsList;
	...
	public void setStatsList(){
		...
		statsList.add(this.getStats().getGamesPlayed());
		statsList.add(this.getStats().getWins());
		statsList.add(this.getStats().getShotsAgainst());
		statsList.add(this.getStats().getGoalsAgainst());
		statsList.add(this.getStats().getSaves());
		statsList.add(this.getStats().getSavePercentage());
		statsList.add(this.getStats().getAvgWinsGP());
	}
	...
	public ArrayList<Object> getStatsList(){
		return statsList;	
	}
	...
	public static ArrayList<Object> printTeamStat(String[] statOptions, int indexOfStat, Roster r){
		...
		ArrayList<Object> sortedStat = new ArrayList<Object>();
		for(Goalie goalie: unsortedStat){
			sortedStat.add(goalie);	
		}
		return sortedStat;
	}
}
...
class Skater extends HockeyPlayer implements Comparable<Skater>{
	...
	public void setStatsList(){
		statsList = new ArrayList<Object>();
		statsList.add(this.getStats().getGamesPlayed());
		statsList.add(this.getStats().getGoals());
		statsList.add(this.getStats().getAssists());
		statsList.add(this.getStats().getPoints());
		statsList.add(this.getStats().getPlusMinus());
		statsList.add(this.getStats().getShots());
		statsList.add(this.getStats().getShootingPercentage());
		statsList.add(this.getStats().getAvgPtsGP());
	}
	...
	public static ArrayList<Object> printTeamStat(String[] statOptions, int indexOfStat, Roster r){
		...
		Collections.sort(unsortedStat);
		ArrayList<Object> sortedStat = new ArrayList<Object>();
		for(Skater skater: unsortedStat){
			sortedStat.add(skater);	
		}
		return sortedStat;
	}
}
...
class Output implements StatsTracking{
	...
	public void setMenuChoices(int choice, int setChoices){
		if(choice == setChoices + 1)
			selectYearMenu();
		else if(choice == setChoices + 2)
			selectTeamOrIndivMenu();
		else if(choice == setChoices + 3){
	...
	public void infoTypeMenu(){
		int setChoices = StatsTracking.infoType.length;
		...
		case 1: rosterDetailsMenu();
			break;
		case 2: playerStatsMenu(StatsTracking.skaterStats.length, "skater", StatsTracking.skaterStats, StatsTracking.numSIntStats, false);
			break;
		case 3: playerStatsMenu(StatsTracking.goalieStats.length, "goalie", StatsTracking.goalieStats, StatsTracking.numGIntStats, true);
			break;
		}
		setMenuChoices(choice, setChoices);
	}
	
	public void rosterDetailsMenu(){
		int setChoices = StatsTracking.teamDetails.length;
		Roster r = new Roster(getSeasonRoster());
		System.out.println("\n********************************************************************");
		...
		rosterDetailsMenu();
			...
			public void getLowHighFiveInteger(int minVal, String statType, ArrayList<Skater> skaterStat, int statSorting, boolean isHighest){
				ArrayList<Integer>lowHighFive = new ArrayList<Integer>();
				if(isHighest == true){
					Collections.reverse(skaterStat);
				}
				lowHighFive.add(minVal); 
		...
		public void sortSomeAllInteger(ArrayList<Object> sStat, int choice){
			...
			System.out.println("MAX = " + (Integer)skaterStat.get(skaterStat.size()-1).getStatsList().get(choice));
			System.out.println("MIN = " + (Integer)skaterStat.get(0).getStatsList().get(choice));
			...
			case 2: getLowHighFiveInteger((Integer)skaterStat.get(0).getStatsList().get(choice), StatsTracking.skaterStats[choice-1], skaterStat, choice, false);
				break;
			case 3: getLowHighFiveInteger((Integer)skaterStat.get(skaterStat.size()-1).getStatsList().get(choice), StatsTracking.skaterStats[choice-1], skaterStat, choice, true);
				break;
		...
		public void sortSomeAllDouble(ArrayList<Object> sStat, int choice){
			...
			case 2: getLowHighFiveDouble((Double)skaterStat.get(0).getStatsList().get(choice), StatsTracking.skaterStats[choice-1], skaterStat, choice, false);
				break;
			case 3: getLowHighFiveDouble((Double)skaterStat.get(skaterStat.size()-1).getStatsList().get(choice), StatsTracking.skaterStats[choice-1], skaterStat, choice, true);
				break;
			case 4: System.out.println("IN PROGRESS");
				break;
			case 5: System.out.print("  You selected: EXIT");
	...
	public void playerStatsMenu(int numChoice, String playerType, String[] statsTrack, int numIntStats, boolean isGoalie){
		int setChoices = numChoice;
		ArrayList<Object> playerStat = null;
		Roster r = new Roster(getSeasonRoster());
		System.out.println("\n********************************************************************");
		System.out.println("WELCOME TO " + playerType.toUpperCase() + " DATA ANALYTICS for the " + getSeason() + "!");
		menuOptions(statsTrack, true);
		int choice = userChoice();
		if(choice <= numChoice){
			playerStat = isGoalie ? Goalie.printTeamStat(statsTrack, (choice-1), r) : Skater.printTeamStat(statsTrack, (choice-1), r);
		}
		if(choice <= numIntStats){
			if(isGoalie){
				sortAll(playerStat, choice);
			}
			else{
				sortSomeAllInteger(playerStat, choice);
			}
		}
		else if(choice > numIntStats & choice <= numChoice){
			if(isGoalie){
				sortAll(playerStat, choice);
			}
			else{
				sortSomeAllDouble(playerStat, choice);
			}
		}	
		setMenuChoices(choice, setChoices);
		playerStatsMenu(numChoice, playerType, statsTrack, numIntStats, isGoalie);
	}
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
public class SortSomeAllV3{
